...
export const ExamPdfViewer: React.FC<ExamPdfViewerProps> = ({ pdfUrl }) => {
  ...
  const isRemoteHttpUrl = typeof pdfUrl === 'string' && /^https?:\/\//i.test(pdfUrl);
  const directOpenUrl = typeof pdfUrl === 'string' ? pdfUrl : null;

  const documentFile = useMemo(() => {
    if (!pdfUrl) return null;

    if (typeof pdfUrl === 'string') {
      return isRemoteHttpUrl
        ? {
          url: pdfUrl,
          httpHeaders: {
    ...
    return pdfUrl;
  }, [isRemoteHttpUrl, pdfUrl]);

  const pageWidth = Math.max(320, Math.min(containerWidth, 1200));
  ...
  const onDocumentLoadError = (error: Error) => {
    console.error('PDF viewer failed to load document', {
      ...
      sourceType: pdfUrl instanceof File ? 'file' : pdfUrl instanceof Blob ? 'blob' : typeof pdfUrl,
    });
  ...
  const updateContainerWidth = useCallback(() => {
    if (scrollContainerRef.current) {
      setContainerWidth(Math.max(scrollContainerRef.current.clientWidth - 64, 320));
    }
  }, []);
  ...
  const errorDescription = loadError?.includes('API version') || loadError?.includes('Worker version')
    ? 'PDF worker không cùng phiên bản với PDF.js API trong frontend. Vui lòng deploy lại bản build mới.'
    : isRemoteHttpUrl
      ? 'File đã được lấy từ storage nhưng PDF.js không xử lý được. Hãy kiểm tra log trình duyệt để xem lỗi chi tiết.'
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between px-6 py-2.5 bg-[#1a1c1e] text-slate-300 border-b border-slate-700/50 shadow-lg z-10">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 bg-[#2b2d31] rounded-lg px-2 py-1 border border-slate-700">
            <button type="button" onClick={handleZoomOut} className="p-1 rounded hover:bg-slate-700 transition-colors" aria-label="Thu nhỏ">
              <ZoomOut className="w-4 h-4" />
            </button>
            <span className="text-[11px] font-medium w-12 text-center text-slate-400">
              {Math.round(zoom * 100)}%
            </span>
            <button type="button" onClick={handleZoomIn} className="p-1 rounded hover:bg-slate-700 transition-colors" aria-label="Phóng to">
      ...
      <div className="flex-1 flex overflow-hidden">
        {numPages > 0 && (
          <aside className="w-44 bg-[#1e2024] border-r border-slate-700/30 flex-col overflow-hidden hidden md:flex">
        ...
        >
          {documentFile ? (
            <Document
              ...
              loading={
                <div className="flex flex-col items-center justify-center min-h-[520px] gap-4 text-slate-400">
                  <Loader2 className="w-10 h-10 animate-spin text-blue-500" />
                  <p className="text-sm font-medium">Đang tải đề thi...</p>
                </div>
              }
              error={
                <div className="flex flex-col items-center justify-center min-h-[520px] gap-4 text-slate-400 p-8 text-center bg-[#2b2d31]">
                  ...
                  <div className="max-w-md">
                    ...
                    {loadError && (
                      <p className="mt-3 rounded-lg bg-slate-950/40 px-3 py-2 text-xs text-slate-500">
                        {loadError}
                      </p>
                    )}
                  ...
                  >
                    ...
                    >
                      <Page
                        ...
                        loading={
                          <div
                            style={{ width: pageWidth * zoom, aspectRatio: '3 / 4' }}
            ...
            <div className="flex flex-col items-center justify-center min-h-[520px] gap-4 text-slate-400 p-8 text-center bg-[#2b2d31]">
              <FileWarning className="w-16 h-16 text-amber-500" />
              <div>
                <h3 className="text-xl font-bold text-white">Chưa có file PDF</h3>
                <p className="text-sm mt-3 text-slate-400">Vui lòng chọn hoặc nhập URL file PDF đề thi.</p>
              </div>
            </div>
          )}
        </div>
      ...
      {!isLoaded && !loadError && documentFile && (
        <div className="pointer-events-none absolute inset-x-0 bottom-0 h-1 bg-blue-500/20">
